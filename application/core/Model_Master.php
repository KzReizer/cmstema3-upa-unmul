...
class Model_Master extends MY_Model
{
	...
	function get_tema($domain)
	{
		$this->db->select('quoteId,quoteTemaId,quoteIsDisplay');
        $this->db->from('f_quote');
        $this->db->join('s_tema','quoteTemaId=temaId','left');
        $this->db->where('quoteIsDisplay', 1);        
        $this->db->limit(1);
        $quote = $this->db->get_compiled_select();

		$this->db->select('*');
		$this->db->from('s_tema');
		$this->db->join('f_footer','footTemaId=temaId','left');
		$this->db->join('f_statistic','statTemaId=temaId','left');
		$this->db->join('(' . $quote . ') dtquote', 'quoteTemaId = temaId', 'LEFT');
		$this->db->where('temaDomain', $domain);
		$qr=$this->db->get();
	...
	function get_header_menu($tema)
	{
		$this->db->select('*');
		$this->db->from('f_page');
		$this->db->where('(pageIsParent="1" or pageIsParent="2")');
		$this->db->where('pageTemaId',$tema);
	...
	function get_child_menu($key,$tema)
	{
		$this->db->select('*');
		$this->db->from('f_page');
		$this->db->where('(pageHeadID = "'.$key.'"  or pageHeadEN = "'.$key.'")');
		$this->db->where('pageIsParent',0);
		$this->db->where('pageTemaId',$tema);
		$this->db->order_by('pageUrut');
		$qr=$this->db->get();
